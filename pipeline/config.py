...
class Config:
    ...
    def setup_logging(self) -> None:
        ...
        configured_file = self.path("logging.file", "output/pipeline.log")
        # One log file per run, not one ever-growing file appended to forever
        # across every run/day — matches the timestamped convention every
        # other run-scoped output already follows (run_report_*.xlsx, tracker
        # files). Timestamp inserted before the extension, computed once here.
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        log_file = configured_file.with_name(f"{configured_file.stem}_{timestamp}{configured_file.suffix}")
