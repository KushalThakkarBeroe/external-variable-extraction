...
def enrich_new_drivers(cfg, llm: LlmHelper, registry_path: Path,
    ...
    # Check required columns. "unit" is deliberately not required: the seed file
    # no longer carries a Unit column (removed 2026-08-07 -- it was unreliable
    # human input, see module docstring) and unit is instead determined during
    # enrichment/connector research, not pre-declared by the analyst.
    required = {"commodity", "driver_name", "region", "start_date"}
    ...
    for _, seed_row in seed_df.iterrows():
        ...
        # No longer sourced from the seed file (Unit column removed 2026-08-07);
        # left blank here and determined later during connector research instead.
        unit = ""

        # Check for missing critical fields
        if not all([commodity, driver_name, region, start_date]):
            log.warning("Seed row skipped: missing critical field(s): %s / %s / %s / %s",
                       commodity, driver_name, region, start_date)
            continue
    ...
    # ---- BIS (FX) / EIA (crude oil, natural gas) secondary backups --------
    # Per senior review: every FRED-sourced FX/crude-oil/natural-gas driver
    # gets an additional backup source, tried only after the existing chain
    # is exhausted. Runs once per invocation, idempotently (skips anything
    # that already has the backup wired), over BOTH pre-existing registry
    # rows and rows just enriched above this run — mirrors the Frankfurter
    # auto-fallback pattern immediately above, as a separate, independent
    # pass so the existing Frankfurter logic is never touched.
    _CRUDE_OIL_EIA_SERIES = {"DCOILBRENTEU": "PET.RBRTE.D", "DCOILWTICO": "PET.RWTC.D"}
    _NATGAS_EIA_SERIES = "NG.RNGWHHD.D"   # Henry Hub (US) — see is_proxy note below

    def _row_get(row_or_dict, key, default=""):
        if isinstance(row_or_dict, dict):
            return row_or_dict.get(key, default)
        return row_or_dict.get(key, default) if hasattr(row_or_dict, "get") else getattr(row_or_dict, key, default)

    def _find_row_by_id(driver_id: str):
        """Locates a driver by id in either registry_df (existing) or new_rows (this pass)."""
        for new_row in new_rows:
            if new_row.get("driver_id") == driver_id:
                return new_row
        matches = registry_df[registry_df["driver_id"] == driver_id]
        if not matches.empty:
            return matches.iloc[0]
        return None

    def _set_fallback(driver_id: str, fallback_id: str) -> None:
        """Sets fallback_driver_id on a row in either new_rows or registry_df."""
        for new_row in new_rows:
            if new_row.get("driver_id") == driver_id:
                new_row["fallback_driver_id"] = fallback_id
                return
        registry_df.loc[registry_df["driver_id"] == driver_id, "fallback_driver_id"] = fallback_id

    def _chain_tail(driver_id: str, max_depth: int = 5) -> str:
        """Walks fallback_driver_id links to the tail (empty fallback_driver_id or missing row)."""
        seen = {driver_id}
        current = driver_id
        for _ in range(max_depth):
            row = _find_row_by_id(current)
            if row is None:
                break
            fb = str(_row_get(row, "fallback_driver_id", "")).strip()
    ...
    def _new_driver_id(base_id: str, suffix: str) -> str:
        candidate = f"{base_id}_{suffix}"
        if candidate not in existing_ids:
            return candidate
        counter = 2
        while f"{base_id}_{suffix}{counter}" in existing_ids:
            counter += 1
        return f"{base_id}_{suffix}{counter}"

    # All FRED and Frankfurter rows in play this run: existing registry rows
    # plus anything just created above (Frankfurter fallbacks + enriched rows).
    fred_candidates = [(r["driver_id"], r) for r in new_rows if r.get("connector") == "fred"]
    if not registry_df.empty:
        fred_candidates += [(row["driver_id"], row) for _, row in
                            registry_df[registry_df["connector"] == "fred"].iterrows()]

    frank_candidates = [(r["driver_id"], r) for r in new_rows if r.get("connector") == "frankfurter"]
    if not registry_df.empty:
        frank_candidates += [(row["driver_id"], row) for _, row in
                             registry_df[registry_df["connector"] == "frankfurter"].iterrows()]

    # BIS: chained onto any Frankfurter fallback row that doesn't yet have
    # its own fallback_driver_id — FRED -> FRANKFURTER -> BIS.
    for frank_id, frank_row in frank_candidates:
        if str(_row_get(frank_row, "fallback_driver_id", "")).strip():
            continue
        currency = str(_row_get(frank_row, "endpoint_or_locator", ""))
        m = re.search(r"target:\s*([A-Za-z]{3})", currency)
        if not m:
            continue
        target_currency = m.group(1).upper()
        bis_id = _new_driver_id(frank_id, "BIS")
        existing_ids.add(bis_id)
        bis_row = {
            "driver_id": bis_id,
            "commodity": str(_row_get(frank_row, "commodity", "")),
            "driver_name": f"{_row_get(frank_row, 'unit', '')} (BIS keyless fallback)",
            "region": str(_row_get(frank_row, "region", "")),
            "extraction_tier": 1,
            "tier_confidence": "High",
            "connector": "bis_fx",
            "source_name": "Bank for International Settlements (BIS) US dollar exchange rates",
            "source_url": "https://stats.bis.org/",
            "endpoint_or_locator": f"currency:{target_currency}",
            "access_mode": "open_api",
            "native_frequency": "Monthly",
            "rollup_method": "mean",
            "unit": str(_row_get(frank_row, "unit", "")),
            "history_from": str(_row_get(frank_row, "history_from", "")) or "",
            "meets_min_history": "Y",
            "update_lag_days": 30,
            "is_proxy": "N",
            "proxy_note": "",
            ...
            "fallback_driver_id": "",
            "data_quality_risk": "Low",
            "transform_hint": "Second-line FX backup, tried after Frankfurter (per senior review)",
            "priority": 3,
            "enrichment_confidence": "",
        }
        new_rows.append(bis_row)
        _register_candidate(bis_row["commodity"], bis_row["region"], bis_id, bis_row["driver_name"])
        _set_fallback(frank_id, bis_id)
        log.info("Auto-created BIS FX fallback %s for Frankfurter row %s", bis_id, frank_id)

    # BIS direct-attach: a FRED FX row can declare a fallback_driver_id
    # ending in _FRANK that does not actually resolve to any row in the
    # registry (a dangling reference — confirmed present for at least
    # DE_SMP_FXNZD/DE_SMP_FXEUR/LA_PLP_FX_FB, predating this change). That is
    # functionally identical to having no fallback at all (cascade.py's
    # fallback step already no-ops when all_specs.get(fallback_driver_id) is
    # None), so replacing the dead pointer with a real, working BIS fallback
    # is a strict improvement, never a regression — nothing was actually
    # being served through the dangling reference before this. Never touches
    # a row whose declared fallback DOES resolve to a real row (handled by
    # the Frankfurter-chaining loop above, or left alone if non-FX).
    _FX_UNIT_RE = re.compile(
        r"(?:USD|EUR|NZD|BRL|GBP|JPY|CHF)/|per (?:USD|EUR|NZD|BRL|GBP|JPY|CHF)", re.IGNORECASE)
    for fred_id, fred_row in fred_candidates:
        unit = str(_row_get(fred_row, "unit", ""))
        if not _FX_UNIT_RE.search(unit):
            continue
        declared_fallback = str(_row_get(fred_row, "fallback_driver_id", "")).strip()
        if not declared_fallback:
            continue  # no fallback declared at all — the pre-existing Frankfurter block already handles this case
        if _find_row_by_id(declared_fallback) is not None:
            continue  # resolves to a real row — already handled above or intentionally something else
        currencies = [p for p in re.split(r"[\s/]", unit.upper()) if len(p) == 3 and p.isalpha()]
        if len(currencies) < 2:
            continue
        target_currency = currencies[0]
        bis_id = _new_driver_id(fred_id, "BIS")
        existing_ids.add(bis_id)
        bis_row = {
            "driver_id": bis_id,
            "commodity": str(_row_get(fred_row, "commodity", "")),
            "driver_name": f"{unit} (BIS keyless fallback)",
            "region": str(_row_get(fred_row, "region", "")),
            "extraction_tier": 1,
            "tier_confidence": "High",
            "connector": "bis_fx",
            "source_name": "Bank for International Settlements (BIS) US dollar exchange rates",
            ...
            "rollup_method": "mean",
            "unit": unit,
            "history_from": str(_row_get(fred_row, "history_from", "")) or "",
            "meets_min_history": "Y",
            "update_lag_days": 30,
            "is_proxy": "N",
            "proxy_note": "",
            ...
            "fallback_driver_id": "",
            "data_quality_risk": "Low",
            "transform_hint": (f"Replaces a dangling fallback_driver_id ({declared_fallback!r}) that did not "
                              f"resolve to any registry row; second-line FX backup (per senior review)"),
            "priority": 3,
            "enrichment_confidence": "",
        }
        new_rows.append(bis_row)
        _register_candidate(bis_row["commodity"], bis_row["region"], bis_id, bis_row["driver_name"])
        _set_fallback(fred_id, bis_id)
        log.warning("FRED FX driver %s declared dangling fallback_driver_id=%r (no such row exists); "
                   "replaced with working BIS fallback %s", fred_id, declared_fallback, bis_id)

    # EIA: crude oil (matched benchmark) and natural gas (Henry Hub proxy,
    # honestly labelled) — attached at the tail of whatever fallback chain
    # the FRED primary already has (none, for every crude-oil/nat-gas FRED
    # row confirmed in the registry today, but this is non-destructive
    # either way — never overwrites an existing fallback).
    _CRUDE_RE = re.compile(r"crude\s*oil|brent|\bwti\b", re.IGNORECASE)
    _NATGAS_RE = re.compile(r"natural\s*gas", re.IGNORECASE)

    for fred_id, fred_row in fred_candidates:
        commodity = str(_row_get(fred_row, "commodity", ""))
        driver_name = str(_row_get(fred_row, "driver_name", ""))
        locator = str(_row_get(fred_row, "endpoint_or_locator", ""))
        series_match = re.search(r"series_id:\s*(\S+)", locator)
        fred_series = series_match.group(1) if series_match else ""

        is_crude = bool(_CRUDE_RE.search(commodity) or _CRUDE_RE.search(driver_name)) or fred_series in _CRUDE_OIL_EIA_SERIES
        is_natgas = bool(commodity.strip().lower() == "natural gas" or _NATGAS_RE.search(driver_name))
        if not is_crude and not is_natgas:
            continue

        tail_id = _chain_tail(fred_id)
        tail_row = _find_row_by_id(tail_id)
        if tail_row is not None and str(_row_get(tail_row, "fallback_driver_id", "")).strip():
            continue  # tail already has a fallback (shouldn't happen post-walk, but never overwrite)
        if tail_row is not None and str(_row_get(tail_row, "connector", "")) == "eia":
            continue  # idempotent: the tail IS already the EIA fallback this pass would add

        if is_crude:
            eia_series = _CRUDE_OIL_EIA_SERIES.get(fred_series, "PET.RBRTE.D")
            eia_id = _new_driver_id(fred_id, "EIA")
            proxy_flag, proxy_note = "N", ""
            source_desc = "EIA crude oil spot price"
        else:
            eia_series = _NATGAS_EIA_SERIES
            eia_id = _new_driver_id(fred_id, "EIA")
            proxy_flag = "Y"
            proxy_note = (
                "EIA Henry Hub (US) natural gas price used as a cross-market proxy backup — "
                "NOT the same regional benchmark as the primary series (e.g. European TTF or "
                "other non-US basis); Henry Hub and other regional gas prices move independently "
                "with a variable basis spread. Flagged for review before being treated as "
                "equivalent data (added per senior review, " + utc_now().strftime("%Y-%m-%d") + ")."
            )
            source_desc = "EIA Henry Hub natural gas spot price (regional proxy)"

        eia_row = {
            "driver_id": eia_id,
            "commodity": commodity,
            "driver_name": f"{driver_name} (EIA fallback)",
            "region": str(_row_get(fred_row, "region", "")),
            "extraction_tier": 4,
            ...
            "endpoint_or_locator": eia_series,
            "access_mode": "api_key",
            "native_frequency": "Daily",
            "rollup_method": "mean",
            "unit": str(_row_get(fred_row, "unit", "")),
            ...
            "fallback_driver_id": "",
            "data_quality_risk": "Low" if is_crude else "Medium",
            "transform_hint": "Backup source, tried after the primary FRED series (per senior review)",
            "priority": 3,
            "enrichment_confidence": "",
        }
        existing_ids.add(eia_id)
        new_rows.append(eia_row)
        _register_candidate(eia_row["commodity"], eia_row["region"], eia_id, eia_row["driver_name"])
        _set_fallback(tail_id, eia_id)
        log.info("Auto-created EIA %s fallback %s for FRED driver %s (chain tail %s)",
                "crude-oil" if is_crude else "natural-gas", eia_id, fred_id, tail_id)
    ...
    if new_rows:
        ...
        if not registry_df.empty:
            ...
            # reindex (not plain [col_order] indexing) so a registry column
            # that no individual new_rows dict happens to set (e.g. a schema
            # column added after those dicts were written) is filled blank
            # instead of raising KeyError — new rows simply get the same
            # blank/default the column already means for every other row.
